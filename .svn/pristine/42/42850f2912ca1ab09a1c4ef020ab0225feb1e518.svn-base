<%-- 
    ...
    Created on : Nov 20, 2017, 6:40:31 PM
    Author     : Cori 5
--%>
...
<script>
    ...
    function displayData() {
        ...
        $tbl.append($('<thead>').append($('<tr>').append(
                ...
                $('<th class="center" width="30%">').html('Hospital Name'),
                $('<th class="center" width="20%">').html('City'),
                $('<th class="center" width="20%">').html('Country'),
                $('<th class="center" width="20%">').html('Hospital Address'),
                $('<th class="center" width="15%" colspan="2">').html('&nbsp;')
                )));
        $.get('clinic.htm?action=getHospitals', {hospitalName: $('#hospitalName').val()},
                function (list) {
    ...
    function saveData() {
        if ($.trim($('#hospitalName').val()) === '') {
            $('#hospitalName').notify('Clinic Name is Required Field', 'error', {autoHideDelay: 15000});
            $('#hospitalName').focus();
            return false;
        }
        
        
         if ($.trim($('#hospitalAddress').val()) === '') {
            $('#hospitalAddress').notify('Clinic Name is Required Field', 'error', {autoHideDelay: 15000});
        ...
        $.post('clinic.htm?action=saveHospital', obj, function (obj) {
    ...
    function deleteRow(id) {
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.post('clinic.htm?action=deleteHospital', {id: id}, function (res) {
...
//        $('#countryId').val('');
    ...
    function editRow(id) {
        $('#hospitalId').val(id);
        $.get('clinic.htm?action=getHospitalById', {hospitalId: id},
                function (obj) {
                    $('#hospitalName').val(obj.TITLE);
                    $('#cityId').val(obj.CITY_ID);
                    $('#countryId').val(obj.COUNTRY_ID);
                    $('#hospitalAddress').val(obj.ADDRESS);
                    $('#addHospital').modal('show');
                }, 'json');
    }
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="hospitalAddress" rows="2" cols="30"></textarea>
                            </div>
                        </div>                        
                    </div>
                </form>
