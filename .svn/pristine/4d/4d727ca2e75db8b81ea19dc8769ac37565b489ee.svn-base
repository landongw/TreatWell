CREATE OR REPLACE FUNCTION GET_HOURS(STTIME IN VARCHAR2,ENTIME IN VARCHAR2, PATTERN IN VARCHAR2) 
RETURN VARCHAR2 
IS
  HOUR1 NUMBER; 
	HOUR2 NUMBER; 
...
BEGIN
  ...
  END IF;
	...
	IF (LENGTH(T4) = 2 AND LENGTH(T5) = 2) THEN	   
	   	CALCHOURS := (TO_CHAR(T4) || '' || PATTERN || TO_CHAR(T5) || '');	     
  
	ELSIF (LENGTH(T4) = 2 AND LENGTH(T5) = 1) THEN 	   
	   	CALCHOURS := (TO_CHAR(T4) || '' || PATTERN || '0' || TO_CHAR(T5) || '');	  
  
	ELSIF (LENGTH(T4) = 1 AND LENGTH(T5) = 1) THEN	   
	   	CALCHOURS := ('0' || TO_CHAR(T4) || '' || PATTERN || '0' || TO_CHAR(T5) || '');
	   
	ELSIF (LENGTH(T4) = 1 AND LENGTH(T5) = 2) THEN
	   	CALCHOURS := ('0' || TO_CHAR(T4) || '' || PATTERN || TO_CHAR(T5) || '');
	END IF;
