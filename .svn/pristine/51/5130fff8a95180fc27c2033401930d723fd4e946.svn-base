CREATE OR REPLACE 
FUNCTION GET_INVOICE_NBR(FP_VOUCHER_DTE IN VARCHAR2, FP_VOUCHER_SUB_TYP IN VARCHAR2, FP_BU_ID IN NUMBER, FP_FIN_YEAR_ID IN NUMBER, FP_COMPANY_ID IN NUMBER)  
RETURN NUMBER IS
...
BEGIN
  ...
  ELSE 
     SELECT MAX(TO_NUMBER(NVL(VM.VOUCHER_NBR,0)))+1 MAX_NBR INTO V_VOUCHER_NBR FROM VOUCHER_MASTER VM, VOUCHER_SUB_TYPE VST 
                 WHERE VM.VOUCHER_SUB_TYP_ID=VST.VOUCHER_SUB_TYP_ID AND VST.VOUCHER_SUB_TYP_DESC=FP_VOUCHER_SUB_TYP
                 AND VM.BU_ID=FP_BU_ID AND VM.FIN_YEAR_ID=FP_FIN_YEAR_ID
                 AND VM.COMPANY_ID= FP_COMPANY_ID;
  END IF;
