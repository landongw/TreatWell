...
<script>
                ...
                function (list) {
                    if (list !== null && list.length > 0) {
                        ...
                        for (var i = 0; i < list.length; i++) {
                            var editHtm = '<i class="fa fa-pencil-square-o" aria-hidden="true" title="Click to Edit" style="cursor: pointer;" onclick="editRow(\'' + list[i].TW_PHARMA_AREA_ID + '\');"></i>';
                            var delHtm = '<i class="fa fa-trash-o" aria-hidden="true" title="Click to Delete" style="cursor: pointer;" onclick="deleteRow(\'' + list[i].TW_PHARMA_AREA_ID + '\');"></i>';
                            if ($('#can_edit').val() !== 'Y') {
                                editHtm = '&nbsp;';
                            }
                            if ($('#can_delete').val() !== 'Y') {
    ...
    function saveData() {
        ...
        $.post('clinic.htm?action=savePharmaArea', obj, function (obj) {
    ...
    function deleteRow(id) {
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.post('clinic.htm?action=deletePharmaArea', {id: id}, function (res) {
    ...
    function addPharmaAreaDialog() {
        $('#areaId').val('');
        $('#title').val('');
        $('#addPharmaArea').modal('show');
    }
    function editRow(id) {
        $('#areaId').val(id);
        $.get('clinic.htm?action=getPharmaAreaById', {areaId: id},
                function (obj) {
...
<div class="modal fade" id="addPharmaArea">
                    ...
                    <div class="row">
                        <div class="col-md-12 text-right" style="padding-top: 23px;">
                            <c:if test="${requestScope.refData.CAN_ADD=='Y'}">
                                <button type="button" class="btn blue" onclick="addPharmaAreaDialog();"><i class="fa fa-plus-circle"></i> New Area</button>
                            </c:if>
                        </div>
                    </div>
                    <br/>
