    ...
    public List<Map> getDashBoardDataForDoctors(String doctorId, String clinicId) {
        ...
        try {
            String query = "SELECT COUNT(TW_PATIENT_ID) TOTAL,'REGISTERED_PATIENT' TITLE FROM TW_PATIENT"
                    ...
                    + " UNION ALL"
                    + " SELECT COUNT(TW_PRESC_LABTEST_ID) TOTAL,'TOTAL_RECOMMANDED' TITLE"
                    + " FROM TW_PRESC_LABTEST WHERE TW_LAB_TEST_ID IS NOT NULL AND TW_PRESCRIPTION_MASTER_ID IN "
                    + " (SELECT TW_PRESCRIPTION_MASTER_ID FROM TW_PRESCRIPTION_MASTER"
                    + " WHERE TW_DOCTOR_ID=" + doctorId + " AND TW_CLINIC_ID=" + clinicId + ") "
                    + " UNION ALL"
