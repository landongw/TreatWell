...
public class SmsServiceImpl implements SmsService {
    ...
    public boolean sendAppointmentMessage(String appointmentId) {
        ...
        try {
            ...
            if (l != null && l.size() > 0) {
                ...
                String email = map.get("EMAIL").toString() + ", " + map.get("EMAIL").toString() + ", " + map.get("EMAIL").toString();
                ...
                if (!email.trim().isEmpty()) {
                    this.emailService.sentAppointmentEmail(message, email.trim());
                }
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    @Override
    public boolean sendAppointmentCancelMessage(String appointmentId) {
        boolean flag = false;
        try {
            String query = "SELECT PAT.PATIENT_NME,PAT.MOBILE_NO,CL.CLINIC_NME,DOC.DOCTOR_NME,TO_CHAR(APP.APPOINTMENT_DTE,'DD-MON-YY')  APPOINTMENT_DTE,"
                    + " TO_CHAR(APP.APPOINTMENT_TIME,'HH:MI AM')  APPOINTMENT_TIME,CI.CITY_NME,CA.AREA_NME,"
            ...
            if (l != null && l.size() > 0) {
                ...
                String clinicName = map.get("CLINIC_NME").toString() + ", " + map.get("AREA_NME").toString() + ", " + map.get("CITY_NME").toString();
                String email = map.get("EMAIL").toString() + ", " + map.get("EMAIL").toString() + ", " + map.get("EMAIL").toString();
                String message = "Your appoinment with " + map.get("DOCTOR_NME").toString() + " has been cancelled. Kindly download our mobile app EZIMEDIC to schedule further appointments.";
                Util.sendCancelAppointmentMessage(map.get("MOBILE_NO").toString(), message);
                if (!email.trim().isEmpty()) {
                    this.emailService.sentCancelAppointmentEmail(message, email.trim());
                }
            }
        } catch (Exception ex) {
            ex.printStackTrace();
