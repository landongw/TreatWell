<%-- 
    Document   : addDiscountRates
    Created on : Jul 16, 2018, 12:13:59 PM
    Author     : farazahmad
--%>
...
<script>
    ...
    function displayData() {
        ...
        $.get('doctor.htm?action=getDiscountCategory', {userType: $('#userType').val()},
                function (list) {
                    if (list !== null && list.length > 0) {
                        $tbl.append($('<tbody>'));
                        for (var i = 0; i < list.length; i++) {
                            var editHtm = '<i class="fa fa-pencil-square-o" aria-hidden="true" title="Click to Edit" style="cursor: pointer;" onclick="editRow(\'' + list[i].TW_DISCOUNT_CATEGORY_ID + '\');"></i>';
                            var delHtm = '<i class="fa fa-trash-o" aria-hidden="true" title="Click to Delete" style="cursor: pointer;" onclick="deleteRow(\'' + list[i].TW_DISCOUNT_CATEGORY_ID + '\');"></i>';
                            if ($('#can_edit').val() !== 'Y') {
                                editHtm = '&nbsp;';
                            }
                            if ($('#can_delete').val() !== 'Y') {
    ...
    function deleteRow(id) {
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.post('doctor.htm?action=deleteDiscountCategory', {id: id}, function (res) {
    ...
    function editRow(id) {
        ...
        $.get('doctor.htm?action=getDiscountCategoryById', {id: id},
                function (obj) {
    ...
    function saveQuestion() {
        ...
        $.ajax({
            url: "doctor.htm?action=saveDiscountCategory",
            type: "POST",
            data: data,
            cache: false,
            dataType: 'json',
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <select id="userType" name="userType" class="form-control" onchange="displayData();">
