<%-- 
    Document   : addMedicineUsage
    Created on : May 31, 2018, 12:02:24 PM
    Author     : farazahmad
--%>
...
<script>
    ...
    function displayData() {
        ...
        $tbl.append($('<thead>').append($('<tr>').append(
                ...
                $('<th class="center" width="70%">').html('Usage'),
                $('<th class="center" width="20%" colspan="2">').html('&nbsp;')
                )));
        $.get('doctor.htm?action=getMedicineUsages', {specialityId: $('#specialityId').val()},
                function (list) {
                    if (list !== null && list.length > 0) {
                        $tbl.append($('<tbody>'));
                        for (var i = 0; i < list.length; i++) {
                            var editHtm = '<i class="fa fa-pencil-square-o" aria-hidden="true" title="Click to Edit" style="cursor: pointer;" onclick="editRow(\'' + list[i].TW_MEDICINE_USAGE_ID + '\');"></i>';
                            var delHtm = '<i class="fa fa-trash-o" aria-hidden="true" title="Click to Delete" style="cursor: pointer;" onclick="deleteRow(\'' + list[i].TW_MEDICINE_USAGE_ID + '\');"></i>';
                            if ($('#can_edit').val() !== 'Y') {
                                editHtm = '&nbsp;';
                            }
                            if ($('#can_delete').val() !== 'Y') {
    ...
    function deleteRow(id) {
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.post('doctor.htm?action=deleteMedicineUsage', {id: id}, function (res) {
    ...
    function editRow(id) {
        ...
        $.get('doctor.htm?action=getMedicineUsageById', {id: id},
                function (obj) {
                    $('#categoryName').val(obj.TITLE);
                    $('#addQuestionCategoryDialog').modal('show');
                }, 'json');
    }
    function saveQuestion() {
        if ($.trim($('#categoryName').val()) === '') {
            $('#categoryName').notify('Title is Required Field', 'error', {autoHideDelay: 15000});
            $('#categoryName').focus();
            return false;
        }
        ///////////
        ...
        $.ajax({
            url: "doctor.htm?action=saveMedicineUsage",
            type: "POST",
            data: data,
            cache: false,
            dataType: 'json',
            ...
            if (data) {
                if (data.result === 'save_success') {
                    $.bootstrapGrowl("Usage saved successfully.", {
                        ele: 'body',
                        type: 'success',
                        offset: {from: 'top', amount: 80},
                        align: 'right',
                    ...
                    $.bootstrapGrowl("Error in Saving Usage. Please try again.", {
                        ele: 'body',
                        type: 'danger',
                        offset: {from: 'top', amount: 80},
                        align: 'right',
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Add Usage</h3>
            </div>
                ...
                <div class="portlet-body">
                    ...
                    <div class="row">
                        <div class="col-md-12 text-right" style="padding-top: 23px; " >
                            <button type="button" class="btn blue" onclick="addQuestionCategory();"><i class="fa fa-plus-circle"></i> Add Usage</button>
                        </div>
                    </div>
                    <div class="row">
