<%-- 
    Document   : viewIntakeQuestions
    Created on : Feb 24, 2018, 12:40:24 PM
    Author     : farazahmad
--%>
...
<script>
    ...
    function displayData() {
        ...
        $.get('performa.htm?action=getIntakeQuestions', {specialityId: $('#specialityId').val()},
                function (list) {
                    if (list !== null && list.length > 0) {
                        $tbl.append($('<tbody>'));
                        for (var i = 0; i < list.length; i++) {
                            var editHtm = '<i class="fa fa-pencil-square-o" aria-hidden="true" title="Click to Edit" style="cursor: pointer;" onclick="editRow(\'' + list[i].TW_INTAKE_MASTER_ID + '\');"></i>';
                            var delHtm = '<i class="fa fa-trash-o" aria-hidden="true" title="Click to Delete" style="cursor: pointer;" onclick="deleteRow(\'' + list[i].TW_INTAKE_MASTER_ID + '\');"></i>';
                            if ($('#can_edit').val() !== 'Y') {
                                editHtm = '&nbsp;';
                            }
                            if ($('#can_delete').val() !== 'Y') {
    ...
    function deleteRow(id) {
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    $.post('performa.htm?action=deleteIntakeQuestion', {id: id}, function (res) {
    ...
    function editRow(id) {
        ...
        $.get('performa.htm?action=getIntakeQuestionById', {id: id},
                function (obj) {
    ...
    function saveQuestion() {
        ...
        $.post('performa.htm?action=saveIntakeQuestion', {specialityId: $('#specialityId').val(),
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Add Intake Question</h3>
            </div>
                ...
                <div class="portlet-body">
                    ...
                    <div class="row">
                        <div class="col-md-12 text-right" style="padding-top: 23px; " >
                            <button type="button" class="btn blue" onclick="addIntakeQuestion();"><i class="fa fa-plus-circle"></i> Add Intake Question</button>
                        </div>
                    </div>
                    <div class="row">
